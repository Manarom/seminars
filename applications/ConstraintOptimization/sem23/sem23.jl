...
using Plots,StaticArrays,Polynomials,Interpolations, RecipesBase, LegendrePolynomials, PlutoUI, LinearAlgebra, PrettyTables, Optimization,OptimizationOptimJL

# ╔═╡ d20e7502-fa03-4448-84f4-dc46acf52c9b
include(raw"PolynomialWrappers.jl")
...
md"Select polynomial basis type for the initial function $(@bind poly_type Select(collect(keys(SUPPORTED_POLYNOMIAL_TYPES)),default = :stand))"
...
begin 
	...
	PolyType = SUPPORTED_POLYNOMIAL_TYPES[poly_type]{polydegree + 1,Float64}
...
begin # bernsteinfit block
	Bern_Type = BernsteinSymPolyWrapper{bern_degree + 1,Float64}
	bern_vander_mat = VanderMatrix(x,Bern_Type)
	(bern_coeffs,bern_fit) = polyfitn(bern_vander_mat,Vector(x),Vector(y_bern))
	ber_max_ocations = bern_max_locations(bern_vander_mat)
end;

# ╔═╡ 1fa31f4b-7ed5-46fa-87ad-e5a4d22d59d6
begin 
	stand_bas = VanderMatrix(x,StandPolyWrapper{bern_degree + 1, Float64})
	(standart_basis_bern_monomial_fit,) = polyfitn(stand_bas,Vector(x),Vector(bern_vander_mat.v[:,sub_monomial_degree +1]))
...
md"Add noise $(@bind demo_data_noise Slider(0.0:1e-2:1, default =  1.0, show_value = true))"
...
@bind  a_lb PlutoUI.combine() do Child
	...
	``a_0`` = $(
		Child(Slider(-3:0.001:3,default=0.67,show_value = true))
	) \
	``a_1`` = $(
		Child(Slider(-3:0.001:3,default=-0.05,show_value = true))
	)\
	``a_2`` = $(
		Child(Slider(-3:0.001:3,default=0.1,show_value = true))
	)\
	``a_3`` = $(
		Child(Slider(-3:0.001:3, default=-0.290,show_value = true))
	)
	"""
end

# ╔═╡ 8674af50-0717-4978-8e8b-94f29b236922
md"""
...
@bind  a_ub PlutoUI.combine() do Child
	...
	``a_1`` = $(
		Child(Slider(-3:0.001:3,default=0.532,show_value = true))
	)\
	``a_2`` = $(
		Child(Slider(-3:0.001:3,default=0.495,show_value = true))
	)\
	``a_3`` = $(
		Child(Slider(-3:0.001:3,default=-0.051,show_value = true))
	)
	"""
end
...
begin
	Bound_Type = BernsteinSymPolyWrapper{length(a_lb),Float64}
	v_data = VanderMatrix(x,Bound_Type)
	a_f_vect = [a_f...]
	y_bern_noisy = (1.0 .+ demo_data_noise*rand(NPOINTS)).*(v_data*a_f_vect);
	
	
	bound_vander_mat = VanderMatrix(x,Bound_Type)
	a_lb_vect = [a_lb...]
	a_ub_vect = [a_ub...]
end;
...
begin 
	...
	probl_unconstr= OptimizationProblem(fun_opt, 
                            starting_vector) 
	a_solve_constr = solve(probl_constr,optimizer)
	a_solve_unconstr = solve(probl_unconstr,optimizer)
	
end

# ╔═╡ 76c60a39-9d68-4765-b698-7c1943572be8
begin
	pp_b = scatter(x,y_bern_noisy,label="data",legendcolumn=3,dpi=600) #1
	
	bern_coeffs_locations = bern_max_locations(bound_vander_mat)
	plot!(pp_b,x,bound_vander_mat*a_lb_vect,label="lower bound",linewidth=3,linecolor=:green)
	scatter!(pp_b,bern_coeffs_locations,a_lb_vect,markercolor=:green,label =nothing, markersize = 6)
	plot!(pp_b,x,bound_vander_mat*a_solve_unconstr,linewidth=3,label = "unconstraint fit",linecolor = :brown)
	plot!(pp_b,x,bound_vander_mat*a_solve_constr,linewidth=3,label = "constraint fit",linecolor = :violet)
	plot!(pp_b,x,bound_vander_mat*a_ub_vect,label="upper bound",linewidth=3,linecolor=:red)
	scatter!(pp_b,bern_coeffs_locations,a_ub_vect,markercolor=:red,label =nothing, markersize = 6)
	scatter!(pp_b,bern_coeffs_locations,a_solve_unconstr,markercolor=:brown,markerstype=:diamond,markersize = 8, label = nothing)
		scatter!(pp_b,bern_coeffs_locations,a_solve_constr,markerstype=:diamond,markersize = 8, label = nothing, markercolor = :violet)
end

# ╔═╡ 6aea3077-0fd9-47ab-8da6-30ef3fc46bda
begin pretty_table(HTML,hcat(a_f_vect,a_lb_vect,a_solve_unconstr,a_solve_constr,a_ub_vect) ,column_labels = ["Data coeffs", "Lower bound", "Unconstraint fit","Constraint fit", "Upper bound"], title = "Constraint optimization")
end

# ╔═╡ 342c4189-1164-4c0f-a4c9-029c8720caea
if PolyType <: BernsteinPolyWrapper || PolyType <: BernsteinSymPolyWrapper
	bern_max(PolyType,1)
	bern_max_values(VanderMatrix(x,PolyType))
	bern_max_locations(VanderMatrix(x,PolyType))
end;
...
Vstand = VanderMatrix(x,StandPolyWrapper{polydegree + 1,Float64});

# ╔═╡ 1f77e0aa-0feb-4add-8f3b-b3c3e4f8db2a
fit_res = polyfitn(Vstand,Vector(x),Vector(y_bern))
...
begin 
	...
	V_stand = VanderMatrix(XXX,StandPolyWrapper{NNN,Float64})
	V_leg = VanderMatrix(XXX,LegPolyWrapper{NNN,Float64})
	V_bern2 = VanderMatrix(XXX,BernsteinSymPolyWrapper{NNN,Float64})
	p_spectra = plot()
	for V in (V_stand,V_leg,V_bern2)
		plot!(p_spectra, svd(V.v).S, label = string(poly_name(V)),linewidth= 3,markersize=8,marker=:auto,markeralpha=0.5)
	end
...
manifest_format = "2.0"
project_hash = "28ace01e163cee77dca12f80ed07733f236d5bb3"
